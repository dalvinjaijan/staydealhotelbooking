...
import { dataForBookingHotel, filterHotelsData, NotifyGetterResponse, searchHotelsData, UnreadMessageCount, userRepositoryInterface } from '../Adapters/interfaces/userInterface/iUserRepositoryInterface'
import UserSchema from '../db/models/userSchema'
import Host from '../db/models/hostSchema'
import Hotel from '../db/models/hotelSchema'
import { bookingHotelDetails, latLng } from '../Adapters/interfaces/userInterface/iUserInteractor'
...
import Rating from '../db/models/ratingSchema'
...
export class userRepository implements userRepositoryInterface {
    ...
    private ratingDb: typeof Rating
    ...
    async searchHotels(updatedData: searchHotelsData): Promise<any> {
        try {
    ...
    async getHotelDetails(data: dataForBookingHotel): Promise<any> {
        try {
            ...
            hotels.forEach((hotel) => {
                hotel.roomCategories = hotel.roomCategories.filter((roomCategory: any) => roomCategory.isAvailable);
            });
            ...
            await Promise.all(
                roomCategory.eachRoomDetails.map(async (room) => {
                    room.BookedDates = room.BookedDates.filter((bookedDate) => 
                        new Date(bookedDate.checkOut) >= currentDate
                    );
            ...
            if (savedBooking && bookingDetails.hotelId) {
                ...
                if (bookingDetails.paymentMethod === 'online') {
                    ...
                    await this.hostDb.updateOne(
                        {
                            "hotels": new mongoose.Types.ObjectId(bookingDetails.hotelId), // Match the host by hotelId in the hotels array
                        },
                        {
    ...
    async fetchWalletTransactions(userId: string): Promise<any> {
        try {
    ...
    async fullRefund(bookingId: string): Promise<string> {
        try {
            ...
            if (bookingDetails && bookingDetails.totalAmount && bookingDetails.checkIn && bookingDetails.checkOut && bookingDetails.noOfRooms) {
                ...
                if(bookingDetails.paymentMethod==="online"){
                    await this.paymentDb.findByIdAndUpdate( bookingDetails.paymentId ,
                        { status: "Refund credited" }, { new: true }
                    )
                ...
                roomCategory.eachRoomDetails.forEach((room) => {
                  if (bookingDetails.noOfRooms && roomsUpdated >= bookingDetails.noOfRooms) return;
                
                  // Filter out matching BookedDates
                  const initialLength = room.BookedDates.length;
                  room.set("BookedDates", room.BookedDates.filter(
                    (date) => !(date.checkIn.getTime() === bookingDetails.checkIn.getTime() &&
    ...
    async partialRefund(bookingId: string): Promise<string> {
        try {
            const bookingDetails = await this.bookingDb.findByIdAndUpdate({ _id: bookingId },
                {
                    bookingStatus: "cancelled"
            ...
            if (bookingDetails && bookingDetails.totalAmount && bookingDetails.checkIn && bookingDetails.checkOut) {
                if(bookingDetails.paymentMethod==="online"){
                    await this.paymentDb.findByIdAndUpdate({ _id: bookingDetails.paymentId },
                        { status: "Partial-refund credited" }, { new: true }
                    )
    ...
    async noRefund(bookingId: string): Promise<string> {
        try {
 ...
 async   rateTheHotel(data: { rating: number; review: string; bookingId: string; userId: string; hotelId: string }): Promise<any> {
    ...
    async fetchTopRatedHotels(latLng: latLng): Promise<any> {
        try {
    ...
    async fetchCoupons(city: string): Promise<any> {
        try {
    ...
    async applyCoupon(code: string, purchaseAmount: number): Promise<any> {
        try {
            const today=new Date()
