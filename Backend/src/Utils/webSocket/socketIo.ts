...
const hostinteractor = new hostInteractor(hostRepo);
type user_and_hosts_socketid = {
  [key: string]: string;
};
const usersAndHostsSocketId: user_and_hosts_socketid = {};
...
export const SocketIntalization = (server: HttpServer) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
      const userIdToDisconnect = Object.keys(usersAndHostsSocketId).find(
        (key) => {
    ...
    socket.on("checkOnlineorNot", ({ userid, hostid, checker }) => {
      if (checker === "host") {
        console.log(
          "usersAndHostsSocketId[userid]",
          usersAndHostsSocketId[userid]
        );
        if (usersAndHostsSocketId[userid]) {
          io.to(usersAndHostsSocketId[hostid]).emit(
      ...
      if (checker === "user") {
        if (usersAndHostsSocketId[hostid]) {
          io.to(usersAndHostsSocketId[userid]).emit(
            "checkedUserIsOnlineOrNot",
            { success: true }
          );
        } else {
    ...
    socket.on(
      ...
      ({ userid, hostid, getter, whomTocall, callerData }) => {
